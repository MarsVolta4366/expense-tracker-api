    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if(authHeader != null) {
            ...
            if(authHeaderArr.length > 1 && authHeaderArr[1] != null) {
                ...
                httpResponse.sendError(HttpStatus.FORBIDDEN.value(), "Authorization header must be in format: Bearer [token].");
                return;
            }
        } else {
            httpResponse.sendError(HttpStatus.FORBIDDEN.value(), "Authorization header is required.");
            return;
        }
